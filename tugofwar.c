...
// Function to get total sum of the array
...
// Function print the subset
void printSubset(int *arr, int *subset, int size){
	for(int i=0; i<size; i++){
		if(subset[i] == 1) printf("%d ", arr[i]);
	}
	printf("\n");
}


// Utility function to solve the Tugg of war problem using backtracking
void tugOfWarUtil(int *arr, int *curr_set, int curr_size, int *best_set, int *min_diff, int total_sum, int curr_sum, int pos, int n){
	// Base case: If we've seelected half the elements
	if(cur_size==n/2){
	...
	curr_set[pos] = 1;
	tugOfWarUtil(arr, curr_set, cur_size+1, best_set,min_diff,total_sum,curr_sum + arr[pos], pos + 1,n);
